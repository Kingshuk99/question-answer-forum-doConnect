...
const Comments = ({answerId, rerender, questionStatus}) => {
    ...
    const deleteComment = async(commentId) => {
        ...
        const isStatusCorrect = response.ok;   ///status === 200 or 201
        if(isStatusCorrect===true){
            const otherComments = comments.filter(comment => comment._id!==commentId);
      ...
      <p>Comments: </p>
        {
            comments.map(comment => (
                <li className="list-group-item" key={comment._id}>
                    ...
                    <div className="card-text">{comment.userId.name}: {comment.statement}
                        <div className="btn-toolbar float-end" role="toolbar" aria-label="Toolbar with button groups">
                            <div className="btn-group me-2" role="group" aria-label="First group">
                            {
                                (sessionInfo && sessionInfo.id===comment.userId._id && questionStatus===true) &&
                                (<button type="button" className="btn btn-secondary float-end"
                                onClick={()=>setEditComment(comment._id)}>Edit</button>)
                            }
                            </div>
                        ...
                        <div className="btn-toolbar float-end" role="toolbar" aria-label="Toolbar with button groups">
                        {(sessionInfo!==null && sessionInfo.role==='user' && questionStatus===true) &&
                        <>
                        <div className="btn-group me-2" role="group" aria-label="First group">
                        <button type="button" className="btn btn-primary" onClick={()=>{likeComment(comment._id)}}>
                        ...
                        <div className="btn-group me-2" role="group" aria-label="Second group">
                        <button type="button" className="btn btn-primary" onClick={()=>{dislikeComment(comment._id)}}>
